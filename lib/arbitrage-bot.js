...
var start = async function (infos) {
    ...
    while (totalMinuteWeight < (process.env.APIMinuteLimit - 23) && process.env.EndSecond > new Date().getSeconds()) {
...
var initArbitrage = async function (infos) {
    ...
    if (tickers.code != 0) {
        ...
        let sleepTime = moment().add(1, "minute").seconds(process.env.StartSecond+1).milliseconds(0).diff(moment());
